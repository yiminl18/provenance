...
def calculate_evidence_length(baseline_name = 'baseline0', dataset_names = ['civic','paper','notice'], model_names = ['gpt4o', 'gpt35', 'gpt4omini']):
        ...
        for dataset_name in dataset_names:
            ...
            for file_path in file_paths:
                ...
                with open(file_path, 'r', encoding='utf-8') as file:
                    ...
                    # provenance_list.append(raw_provenance)
    ...
    # print(f"ave raw provenance token number: {get_token_num_for_list(raw_provenance)}")
    # print(f"evidence token number: {get_token_num_for_list(evidence)}")
    # print(f"compression rate: {get_token_num_for_list(evidence)/get_token_num_for_list(raw_provenance)}")
    print(f"average provenance token number: {sum(length_of_provenance)/len(length_of_provenance)}")
    print(f"average evidence token number: {sum(length_of_evidence)/len(length_of_evidence)}")
    print(f"average ration of evidence token number to provenance token number: {sum(length_of_evidence) / sum(length_of_provenance)}")
